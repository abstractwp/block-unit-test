...
var buildFiles = ['./**', 'imgs/**/*.{gif,jpg,png,svg}', '!build', '!build/**', '!node_modules/**', '!*.json', '!*.map', '!*.xml', '!gulpfile.js', '!*.sublime-project', '!*.sublime-workspace', '!*.sublime-gulp.cache', '!*.log', '!*.DS_Store', '!*.gitignore', '!TODO', '!*.git', '!*.ftppass', '!*.DS_Store', '!sftp.json', '!yarn.lock', '!*.md', '!package.lock'];
...
gulp.task('release-notice', function (done) {
	return gulp.src('./')
		.pipe(notify({
			message: 'The v' + pkg.version + ' release of ' + title + ' has been built.',
			onLast: false
		}))
	done();
});

gulp.task('build', gulp.series('clearCache', 'clean', 'updateVersion', 'cleanSrc', 'variables', 'translate', 'copy', 'zip', 'release-notice', function (done) {
	done();
}));
